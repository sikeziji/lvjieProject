...
/**
 * Created by zwj on 2017/12/4.
 */

public class DevLog {

    private DataDBHelper LogDBHelper;

    public DevLog(Context context) {
        LogDBHelper = DataDBHelper.getInstance(context);
    }

    public void Add(String compName, String content, ErrorLog.msgType logType) {
    ...
    private static void AddDevOpeationLog(String compName, String content, ErrorLog.msgType logType) {
        RecMessage RsMessage = new RecMessage();
        try {
            RsMessage.setMesType(logType);
            RsMessage.setMessage(compName + "_" + content + "_" + logType);
        } catch (Exception e) {
            saveExceptInfo2File("组分[" + compName + "] AddDevOpeationLog：" + content + e.toString());
        }
     ...
     */
    public static void saveOperationLogDataModifyMsg(String compName, String Key, String newValue, String msgInfo, ErrorLog.msgType logType) {
        try {
            if (!getConfigData(compName, Key).equals(newValue)) {
                AddDevOpeationLog(compName, msgInfo + "" + newValue, logType);
            }
        } catch (Exception e) {
            saveExceptInfo2File("保存组份参数至数据库变量内容异常" + e.toString());
        }
    }
     ...
     * Key : 数据库的参数key  newValue:要存储的数据库值  日志保存的描述
     */
    public static void saveOperationLogPublicDataModifyMsg(String compName, String Key, String newValue, String msgInfo, ErrorLog.msgType logType) {
        try {
            if (!getPublicConfigData(Key).equals(newValue)) {
                AddDevOpeationLog(compName, msgInfo + "" + newValue, logType);
            }
        } catch (Exception e) {
            saveExceptInfo2File("保存公共参数至数据库变量内容异常" + e.toString());
        }
    }
     ...
     * 报错操作文字描述内容
     */
    public static void saveOperationLogMsg(String compName, String msgInfo, ErrorLog.msgType logType) {
        try {
            AddDevOpeationLog(compName, msgInfo, logType);
        } catch (Exception e) {
            saveExceptInfo2File("保存操作日志异常" + e.toString());
        }
    }
}
