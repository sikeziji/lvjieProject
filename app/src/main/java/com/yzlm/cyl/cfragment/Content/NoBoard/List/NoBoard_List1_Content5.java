...
public class NoBoard_List1_Content5 extends SubFragment {
    private static NoBoard_List1_Content5 fragment = null;
    private int index = 0;
    ...
    private NoBoardErrorLog.msgType[] types = {NoBoardErrorLog.msgType.报错_警告, NoBoardErrorLog.msgType.报错_错误};
    ...
    public static NoBoard_List1_Content5 newInstance() {
        if (fragment == null) {
            fragment = new NoBoard_List1_Content5();
        }
        return fragment;
    }
    ...
    protected int getLayoutResId() {
        return R.layout.no_board_list1_content5;
    }
                ...
                export = new Thread(new Runnable() {
                    ...
                    public void run() {
                        try{
                            ...
                            List<Map> allList = mError.selectAlarmAll(mNoBoardCompName, null, null, types, 0);
                            if (allList.size() > 0) {
                                ...
                                if (extSdcard != null) {
                                    ...
                                    String[] title = new String[]{getString(R.string.time), getString(R.string.error_num), getString(R.string.info)};
                                    ExcelUtils.createExcelAndTitle(file.toString() + "/" + mNoBoardCompName + "Alarm_" + simpleDateFormat.format(Calendar.getInstance().getTime()) + ".xls", getString(R.string.alarm), title);
                                    ExcelUtils.writeObjListToExcel(ErrorHandler, allDataItems, extSdcard + "waterAlarm" + "/" + mNoBoardCompName + "alarm_" + simpleDateFormat.format(Calendar.getInstance().getTime()) + ".xls", context);
                                } else {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case 500: {
                    ...
                    if (timerMsg == null) {
                        saveNoBoardOperationLogMsg(mNoBoardCompName, "报警记录导出", ErrorLog.msgType.操作_信息);
                        timerMsg = new Timer();
    ...
    private void refreshData() {
        List<Map> list = mError.select(mNoBoardCompName, startTime, endTime, types, index);
        if (list.size() == 0) {
            tv.setText("");
            index -= 100;
        } else {
    ...
    private class Select implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            st.setTargetFragment(NoBoard_List1_Content5.this, 1);
