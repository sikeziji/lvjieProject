 ...
 */
public class Display_swdx extends SubFragment {
    private Callbacks mCallbacks;
    public static List<LinearLayout> Ld_swdx = new ArrayList<>();
    private static Display_swdx fragment = null;
    public static TextView mTxTControlDevicesInfo;


    public static Display_swdx newInstance() {
        if (fragment == null) {
            fragment = new Display_swdx();
        }
        return fragment;
    }
    ...
    protected void DoThings() {
        ...
        for (int i = 0; i < Ld_swdx.size(); i++) {
            ...
            if (i < compsAndMeasParam) {
                Ld_swdx.get(index).setVisibility(View.VISIBLE);
                LayoutInflater inflater = getActivity().getLayoutInflater();
                if (Ld_swdx.get(index).findViewById(R.id.display_swdx_item) == null) {
                    Ld_swdx.get(index).addView(inflater.inflate(R.layout.display_swdx_item, null));
                }
                if (i >= strComponent.get(1).length) {
                    //无背板 5722参数显示
                    String compName = strNoBoardComponent.get(1)[i - strComponent.get(1).length];
                    ((TextView) Ld_swdx.get(index).findViewById(R.id.Name)).setText(compName);
                    ...
                    //((TextView) Ld_swdx.get(index).findViewById(R.id.tvLC)).setText(YBLCL + "-" + YBLCH);
                    ...
                    ((TextView) Ld_swdx.get(index).findViewById(R.id.tvSYLC)).setText(DQLCL + "-" + DQLCH);

                    Ld_swdx.get(index).setOnClickListener(new ld2Click());
                    index++;
                } else {
                    // 有背板组分 5721
                    ...
                    Ld_swdx.get(index).setOnClickListener(new ldClick());
                    TextView txtState = Ld_swdx.get(i).findViewById(R.id.tv_workSate);
                    if (txtState == null) {
                        txtState = Ld_swdx.get(i).findViewById(i);
                    }
                ...
                Ld_swdx.get(index++).setVisibility(View.GONE);
            }
        }
        ...
        public void onClick(View view) {
            try {
                ...
                if (!workState.get(name).equals(context.getString(R.string.normal)) && !workState.get(name).equals(context.getString(R.string.off_line))
                    ...
                    st.setTargetFragment(Display_swdx.this, 1);
                    st.show(fm, "alert-ok");
                }
            } catch (Exception e) {
            }
